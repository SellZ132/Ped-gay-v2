...
# =========================================================
# การตั้งค่าโทเค่นบอท (รองรับหลายโทเค่น)
# =========================================================
# วิธีใส่โทเค่นหลายตัว: ตั้งค่า Environment Variables ดังนี้
#   BOT_TOKEN_1 = โทเค่นบอทตัวที่ 1
#   BOT_TOKEN_2 = โทเค่นบอทตัวที่ 2
#   BOT_TOKEN_3 = โทเค่นบอทตัวที่ 3
#   ... (เพิ่มได้เรื่อยๆ)
# หรือถ้าใช้บอทตัวเดียว ใส่แค่ BOT_TOKEN ก็ได้
# =========================================================
...
def get_tokens():
    ...
    i = 1
    while True:
        t = os.environ.get(f'BOT_TOKEN_{i}', '').strip()
        if not t:
            break
        tokens.append(t)
        i += 1
...
def create_bot(bot_index=0):
    """สร้าง instance บอทใหม่พร้อม command ครบชุด"""

    config_file = "config.json" if bot_index == 0 else f"config_{bot_index}.json"
    ...
    class MyBot(commands.Bot):
        def __init__(self):
            ...
            self.ai_channel_id = 1486801910433448007
            self.blocked_ai_users = []
            self.astd_channel_id = None
            self.astd_message_id = None
            self.banner_channel_id = None
            self.banner_x_units = "ยังไม่มีข้อมูล (ดูได้จากลิงก์สด)"
            self.banner_y_units = "ยังไม่มีข้อมูล (ดูได้จากลิงก์สด)"
            self.banner_z_units = "ยังไม่มีข้อมูล (ดูได้จากลิงก์สด)"
            self.dot_role_channel_id = cfg.get("dot_role_channel_id")
            self.dot_role_id = cfg.get("dot_role_id")
            self.dot_log_channel_id = cfg.get("dot_log_channel_id")
        ...
        async def setup_hook(self):
            try:
                ...
                print(f"✅ Sync Commands to Guild Completed!")
            except discord.errors.Forbidden:
                await self.tree.sync()
                print(f"✅ Sync Commands Globally Completed!")
    ...
    async def on_ready():
        ...
        print(f'🤖 AI Channel ID: {bot.ai_channel_id}')
    ...
    async def on_message(message: discord.Message):
        ...
        if bot.dot_role_channel_id and message.channel.id == bot.dot_role_channel_id and message.content.strip() == ".":
            if bot.dot_role_id:
                role = message.guild.get_role(bot.dot_role_id)
                if role and role not in message.author.roles:
                    try:
                        await message.author.add_roles(role, reason="พิมพ์ . ในห้องรับยศ")
                        if bot.dot_log_channel_id:
                            log_channel = bot.get_channel(bot.dot_log_channel_id)
                            if log_channel:
                                embed = discord.Embed(
                                    title="✅ ให้ยศสำเร็จ",
        ...
        if bot.ai_channel_id is None or message.channel.id != bot.ai_channel_id:
            await bot.process_commands(message)
            return

        if message.author.id in bot.blocked_ai_users:
    ...
    async def astd_auto_update():
        if bot.astd_channel_id is None:
            return
        channel = bot.get_channel(bot.astd_channel_id)
        if channel is None:
            return
        embed = await build_astd_embed()
        try:
            if bot.astd_message_id:
                msg = await channel.fetch_message(bot.astd_message_id)
                await msg.edit(embed=embed)
            else:
                msg = await channel.send(embed=embed)
                bot.astd_message_id = msg.id
        except Exception:
            msg = await channel.send(embed=embed)
            bot.astd_message_id = msg.id
    ...
    async def banner_notify_task():
        if bot.banner_channel_id is None:
            return
        now = datetime.now(timezone.utc)
        if now.second > 15:
            return
        if now.minute != 0:
            return
        channel = bot.get_channel(bot.banner_channel_id)
        if channel is None:
            return
        embed = discord.Embed(
        ...
        embed.add_field(name="🟡 Banner X", value=bot.banner_x_units, inline=False)
        embed.add_field(name="🔵 Banner Y", value=bot.banner_y_units, inline=False)
        embed.add_field(name="🔴 Banner Z", value=bot.banner_z_units, inline=False)
    ...
    async def setup_dot_role(interaction: discord.Interaction, channel: discord.TextChannel, role: discord.Role, log_channel: discord.TextChannel):
        ...
        bot.dot_role_channel_id = channel.id
        bot.dot_role_id = role.id
        bot.dot_log_channel_id = log_channel.id
    ...
    async def check_dot_role(interaction: discord.Interaction):
        if interaction.user.id not in ALLOWED_USERS:
            await interaction.response.send_message("❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้", ephemeral=True)
            return
        if bot.dot_role_channel_id and bot.dot_role_id and bot.dot_log_channel_id:
            ch = interaction.guild.get_channel(bot.dot_role_channel_id)
            role = interaction.guild.get_role(bot.dot_role_id)
            log_ch = interaction.guild.get_channel(bot.dot_log_channel_id)
            await interaction.response.send_message(
                f"✅ ระบบรับยศอัตโนมัติ **เปิดอยู่**\n"
                f"📢 ห้องรับยศ: {ch.mention if ch else bot.dot_role_channel_id}\n"
                f"🏷️ ยศที่จะให้: {role.mention if role else bot.dot_role_id}\n"
                f"📋 ห้อง Log: {log_ch.mention if log_ch else bot.dot_log_channel_id}",
                ephemeral=True
            )
        else:
    ...
    async def remove_dot_role(interaction: discord.Interaction):
        if interaction.user.id not in ALLOWED_USERS:
            ...
            return
        await interaction.response.defer(ephemeral=True)
        bot.dot_role_channel_id = None
        bot.dot_role_id = None
        bot.dot_log_channel_id = None
        save_config(config_file, {})
        try:
    ...
    async def setbanner_channel(interaction: discord.Interaction, channel: discord.TextChannel):
        if interaction.user.id not in ALLOWED_USERS:
            await interaction.response.send_message("❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้", ephemeral=True)
            return
        bot.banner_channel_id = channel.id
        embed = discord.Embed(
    ...
    async def setup_astd(interaction: discord.Interaction, channel: discord.TextChannel):
        if interaction.user.id not in ALLOWED_USERS:
            await interaction.response.send_message("❌ คุณไม่มีสิทธิ์ใช้คำสั่งนี้", ephemeral=True)
            return
        bot.astd_channel_id = channel.id
        bot.astd_message_id = None
...
async def run_bot_with_retry(token, index):
    while True:
        bot = create_bot(index)
        try:
            await bot.start(token)
        except discord.errors.HTTPException as e:
            if e.status == 429:
                print(f"⚠️ Bot {index+1}: Rate limited ขณะ login รอ 60 วินาทีแล้วลองใหม่...")
                await asyncio.sleep(60)
            else:
                raise
        except Exception as e:
            print(f"❌ Bot {index+1}: เกิดข้อผิดพลาด: {e} — รอ 30 วินาทีแล้วลองใหม่...")
...
async def run_all_bots(tokens):
    print(f"🚀 กำลังเริ่มต้น {len(tokens)} บอท...")
    await asyncio.gather(*[run_bot_with_retry(token, i) for i, token in enumerate(tokens)])


if __name__ == "__main__":
    tokens = get_tokens()
    if not tokens:
        print("❌ ไม่พบโทเค่นบอทใดๆ!")
        print("กรุณาตั้งค่า Environment Variables:")
        print("  BOT_TOKEN_1 = โทเค่นบอทตัวที่ 1")
        print("  BOT_TOKEN_2 = โทเค่นบอทตัวที่ 2")
        print("  ... (เพิ่มได้เรื่อยๆ)")
        print("หรือใช้ BOT_TOKEN สำหรับบอทตัวเดียว")
    else:
        print(f"✅ พบโทเค่น {len(tokens)} ตัว")
        keep_alive()
        asyncio.run(run_all_bots(tokens))
